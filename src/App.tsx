...
export default function App() {
  ...
  const handleCycleTheme = () => {
    ...
    cycleTheme()
    dispatch({ type: 'SET_MESSAGE', message: t(`common.theme_${next}`) })
  }
  ...
  return (
    <div className="min-h-screen bg-rice-100 paper-noise dark:bg-charcoal-900">
      ...
      <nav className="safe-bottom fixed bottom-0 left-0 right-0 z-30 grid grid-cols-4 border-t border-charcoal-900/5 bg-white/95 px-2 pt-2 backdrop-blur dark:border-rice-200/10 dark:bg-charcoal-900/95 lg:hidden">
...
function MobileNav({ active, icon: Icon, label, badge, onClick }: { active: boolean; icon: typeof MenuIcon; label: string; badge?: number; onClick: () => void }) {
  return (
    <button onClick={onClick} className={`relative flex flex-col items-center gap-1 rounded-xl py-2 text-xs font-semibold transition ${active ? 'bg-chili-50 text-chili-500 dark:bg-chili-700/20 dark:text-chili-100' : 'text-charcoal-500 dark:text-rice-200/70'}`}>
