          ...
          return (
            <article key={product.id} className="group animate-rise overflow-hidden rounded-3xl border border-charcoal-900/5 bg-white shadow-card transition hover:-translate-y-1 dark:border-rice-200/10 dark:bg-charcoal-700" style={{ animationDelay: `${index * 40}ms` }}>
              <div className="relative h-40 overflow-hidden">
                ...
                {unavailable && <span className="absolute inset-0 flex items-center justify-center bg-white/55 text-lg font-extrabold text-charcoal-900 backdrop-blur-sm dark:bg-charcoal-900/55 dark:text-rice-200">{t('menu.sold_out')}</span>}
              ...
              <div className="p-4"><div className="flex items-start justify-between gap-3"><div><h3 className="font-extrabold text-charcoal-900 dark:text-rice-100">{t(product.name)}</h3><p className="mt-1 line-clamp-1 text-xs text-charcoal-500 dark:text-rice-200/70">{t(product.description)}</p></div><Button size="icon" disabled={unavailable} onClick={() => openSpec(product)} className="h-10 w-10 shrink-0 rounded-full"><Plus size={18} /></Button></div><p className="mt-4 text-xl font-extrabold text-chili-500">{money(product.price)} <small className="text-xs font-medium text-charcoal-500 dark:text-rice-200/70">{t('menu.from')}</small></p></div>
            </article>
          )
        })}
      </div>
      ...
      <Dialog open={!!selected} onOpenChange={(open) => { if (!open) { setSelected(null); setShowRiskWarning(false) } }}>
        <DialogContent title={selected ? t(selected.name) : ''}>
          {selected && <>
            ...
            <div className="mt-5"><p className="mb-3 flex items-center gap-2 text-sm font-bold text-charcoal-900 dark:text-rice-100"><Users size={16} />{t('menu.who_ordered')}</p><div className="flex gap-2">{diners.map((name) => <button key={name} onClick={() => setDiner(name)} className={`rounded-full px-4 py-2 text-sm font-bold ${diner === name ? 'bg-amber-400 text-charcoal-900' : 'bg-white text-charcoal-500 dark:bg-charcoal-700 dark:text-rice-200/70'}`}>{name}</button>)}</div></div>
      ...
      <Dialog open={showRiskWarning} onOpenChange={(open) => !open && cancelSuperSpicy()}>
        <DialogContent title={t('menu.super_spicy_warning_title')}>
          <div className='mt-4 flex flex-col items-center gap-4'>
            <span className='flex h-14 w-14 items-center justify-center rounded-2xl bg-chili-50 text-chili-500 dark:bg-chili-700/20 dark:text-chili-100'><AlertTriangle size={28} /></span>
